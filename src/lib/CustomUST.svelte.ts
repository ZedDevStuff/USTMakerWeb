...
export class CustomUST {
    ...
    levels: SvelteMap<string, SvelteMap<string, string>> = new SvelteMap();
    ...
    static getTemplate(): CustomUST {
        let standard: SvelteMap<string, string> = this.templateLevel(["clean", "battle"]);
        let standardTwo: SvelteMap<string, string> = this.templateLevel(["clean1", "clean2", "battle1", "battle2"]);

        let ust = new CustomUST("Name", "Author", "Description");
        
        ust.levels.set("global", new SvelteMap([["audioClipName", "relative/path/to/audio"]]));
    ...
    private static templateLevel(entries: string[]) {
        let level = new SvelteMap<string, string>();
    ...
    static fromJson(json: string): CustomUST | null {
        ...
        for (let level in keys) {
            ...
            let levelMap = new SvelteMap<string, string>();
